...
#ifndef CONTAINERS_MULTISET_HPP
#define CONTAINERS_MULTISET_HPP
...
namespace ft {
	...
	class multiset : public BinaryTreeSet<Key, Compare> {
	...
	public:
		...
		multiset() : BinaryTreeSet<Key, Compare>(true), _size(0) {}
		explicit multiset( const Compare& comp, const Allocator& alloc = Allocator() ) : BinaryTreeSet<Key, Compare>(comp, true),
																						 _size(0), _allocator(alloc) {}
		template< class InputIt >
		multiset( InputIt first, InputIt last, const Compare& comp = Compare(), const Allocator& alloc = Allocator() ) : BinaryTreeSet<Key, Compare>(comp, true),
																														 _size(0), _allocator(alloc)
		{
//			std::pair<T, bool> pair;
			insert(first, last);
		}
		multiset( const multiset& other ) : BinaryTreeSet<Key, Compare>(true), _size(0) {
			*this = other;
		}
		~multiset() {}
		...
		size_type count( const Key& key ) const {
			...
			while (it != end())
			{
				if (!this->_comparator(key, *it) && !this->_comparator(*it, key))
					++count;
				else if (this->_comparator(key, *it))
					break;
				++it;
			}
			return (count);
		...
		ft::pair<iterator,iterator> equal_range( const Key& key ) {
			...
			iterator tmp = it;
			while (tmp != end() && (!this->_comparator(key, *tmp) && !this->_comparator(*tmp, key)))
				++tmp;
		...
		ft::pair<const_iterator,const_iterator> equal_range( const Key& key ) const {
			...
			while (tmp != end() && !this->_comparator(key, *it) && !this->_comparator(*it, key))
				++tmp;
			return (ft::make_pair(it, tmp));
		}
	...
	{
		typename ft::multiset<Key, Compare, Alloc>::const_iterator it_lhs = lhs.begin();
		typename ft::multiset<Key, Compare, Alloc>::const_iterator  it_rhs = rhs.begin();

...
#ifndef CONTAINERS_BINARY_TREE_SET_HPP
#define CONTAINERS_BINARY_TREE_SET_HPP
...
protected:
	...
	explicit BinaryTreeSet(bool repeat = false) : _NULL(new Node), _root(_NULL), _min(_NULL),
											   _max(_NULL), _is_key_repeated(repeat) {}
	explicit BinaryTreeSet(const Compare &comp, bool repeat = false) : _NULL(new Node), _root(_NULL),  _min(_NULL),
	...
	ft::pair<Node*, bool> insert_key(const Key &key)
	{
		if (_root == _NULL)
		{
			_root = new Node(key, _NULL);
	...
	ft::pair<Node*, bool> insert(const Key &key, Node *leaf)
	{
	...
	void case_red_or_black_leaf_two_childs(Node *leaf)
	{
		...
		if (_min == max_left_element)
			_min = leaf;
		const Key tmp = max_left_element->_key;
		Key *non_cast_max_elem = const_cast<Key*>(&max_left_element->_key);
		*non_cast_max_elem = leaf->_key;
		Key *non_cast_leaf_pair = const_cast<Key*>(&leaf->_key);
		*non_cast_leaf_pair = tmp;
...
//				show();
	...
	void show(Node *leaf, int level)
	{
		...
		for (int i = 0; i < level; ++i)
			std::cout << "       ";
		if (leaf->_color == RED_L)
			std::cout << RED << leaf->_key << RESET << std::endl;
		else
			std::cout << GREEN << leaf->_key << RESET << std::endl;
		if (leaf->_left != _NULL)

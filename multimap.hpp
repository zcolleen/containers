...
namespace ft {
	...
	class multimap : public BinaryTree<Key, T, Compare> {
	...
	public:
		...
		multimap() : BinaryTree<Key, T, Compare>(true), _size(0) {}
		explicit multimap( const Compare& comp, const Allocator& alloc = Allocator() ) : BinaryTree<Key, T, Compare>(comp, true),
		        _size(0), _allocator(alloc) {}
		template< class InputIt >
		multimap( InputIt first, InputIt last, const Compare& comp = Compare(), const Allocator& alloc = Allocator() ) : BinaryTree<Key, T, Compare>(comp, true),
		        _size(0), _allocator(alloc)
		{
//			std::pair<T, bool> pair;
			insert(first, last);
		}
		multimap( const multimap& other ) : BinaryTree<Key, T, Compare>(true), _size(0) {
			*this = other;
		}
		~multimap() {}
	...
	{
		typename ft::multimap<Key, T, Compare, Alloc>::const_iterator it_lhs = lhs.begin();
		typename ft::multimap<Key, T, Compare, Alloc>::const_iterator  it_rhs = rhs.begin();
